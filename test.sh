...
# Charger la SECRET_KEY depuis le fichier .env (à la racine)
if [ -f .env ]; then
  export SECRET_KEY=$(grep '^SECRET_KEY=' .env | cut -d '=' -f2-)
else
  echo ".env introuvable : impossible de récupérer SECRET_KEY"
  exit 1
fi

# Générer dynamiquement un token JWT valide
